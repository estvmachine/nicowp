...
jQuery(document).ready(function($) {  
    ...
    $('.select-color-scheme').change(function() {
        ...
        var fields_to_change = $(this).closest('.select_wrapper').find('.color-fields-to-change').text();
        fields_to_change = $.parseJSON(fields_to_change);
        
        for (index = 0; index < fields_to_change.length; ++index) {
            if(color) {
                of_update_color(fields_to_change[index], color);
            }
        }
    });
    ...
    $('#custom_color_scheme_button').on('click', function() {
        ...
        var fields_to_change = $('#color_scheme').closest('.select_wrapper').find('.color-fields-to-change').text();
        fields_to_change = $.parseJSON(fields_to_change);
