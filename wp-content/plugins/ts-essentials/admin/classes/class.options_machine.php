	...
	public function Menu() {
		...
		foreach ($options as $value) {
			...
			if($value['type'] == 'heading') {
                ...
                echo '<li class="'. esc_attr($header_class) .'"><a title="'. esc_attr($value['name']) .'" href="#'. esc_attr($jquery_click_hook) .'">'. strip_tags($value['name'] .$caption, '<span>') .'</a></li>';
            }
            else
            {
                continue;
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'select_color_scheme':
					$mini ='';
					if(!isset($value['mod'])) $value['mod'] = '';
					if(!isset($value['fields']) || !is_array($value['fields'])) $value['fields'] = array();
					if($value['mod'] == 'mini') { $mini = 'mini';}
					$output .= '<div class="select_wrapper ' . esc_attr($mini) . '">';
            ...
            echo strip_tags($output, $allowed_tags); // new
