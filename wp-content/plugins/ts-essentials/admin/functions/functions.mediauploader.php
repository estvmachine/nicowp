...
if ( ! function_exists( 'optionsframework_mlu_js' ) ) {
	...
	function optionsframework_mlu_js () {
		...
		wp_register_script( 'of-medialibrary-uploader', TS_ESSENTIALS_ADMIN_URI .'assets/js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
...
if ( ! function_exists( 'optionsframework_mlu_get_silentpost' ) ) {
	...
	function optionsframework_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$query = 'SELECT ID FROM ' . esc_sql($wpdb->posts) . ' WHERE post_parent = 0';
			foreach ( $_args as $k => $v ) {
				$query .= ' AND ' . esc_sql($k) . ' = "' . esc_sql($v) . '"';
			} // End FOREACH Loop
