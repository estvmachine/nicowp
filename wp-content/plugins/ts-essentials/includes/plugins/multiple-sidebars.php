...
class sidebar_generator {
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'=>$sidebar,
					'id' => 'ts-custom-sidebar-'.strtolower($sidebar_class),
	...
	public static function add_sidebar(){
		...
		echo '<script type="text/javascript">jQuery(\'#sbg_table\').append(\'<tr id="sbg-row-'.esc_js($id).'"><td>'.esc_js($name).'</td><td>'.esc_js($id).'</td><td><a href="javascript:void(0)" onclick="remove_sidebar_link(\\\''.esc_js($name).'\\\',\\\''.esc_js($id).'\\\')">remove</a></td></tr>\')</script>';		
		exit;
	}
	...
	public static function admin_page(){
		...
		<div class="wrap">
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				?>
				<tr class="<?php echo esc_attr($alt)?>" id="sbg-row-<?php echo esc_attr($id);?>">
					<td><?php echo esc_attr($sidebar); ?></td>
					<td><?php echo esc_attr($id); ?></td>
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_js($sidebar); ?>','<?php echo esc_js($id); ?>');" title="Remove this sidebar"><?php _e('remove', 'ThemeStockyard');?></a></td>
				</tr>
				<?php
						$cnt++;
					}
