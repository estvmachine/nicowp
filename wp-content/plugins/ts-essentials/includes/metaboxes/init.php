...
function cmb_meta_boxes_init()
{
    $meta_boxes = array();
    $meta_boxes = apply_filters( 'cmb_meta_boxes' , $meta_boxes );
    
    //die('<pre>'.print_r($meta_boxes, true).'</pre>'); // testing
    
    foreach ( $meta_boxes as $meta_box ) {
        $my_box = new cmb_Meta_Box( $meta_box );
    }
}
add_action( 'init', 'cmb_meta_boxes_init', 200);
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
            ...
            ?>
                ...
                jQuery(function($) {
                    ...
                    if(jQuery.inArray( jQuery('#<?php echo esc_attr($field['unhide_id']);?>').val(), <?php echo json_encode($field['unhide_value']);?> ) >= 0) {
            ...
            $cmb_hidden_style = ($field['hidden'] === true) ? 'display:none' : '';
			echo '<tr id="cmb_field_row_'.esc_attr($field['id']).'" style="'.esc_attr($cmb_hidden_style).'">';

			if ( $field['type'] == "title" ) {
				echo '<td colspan="2">';
			} else {
				if( $this->_meta_box['show_names'] == true ) {
					echo '<th style="width:18%"><label for="', esc_attr($field['id']), '">', wp_kses_post($field['name']), '</label></th>';
				}
				echo '<td>';
			}
			...
			switch ( $field['type'] ) {
				...
				case 'text':
					echo '<input type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" />','<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				case 'text_small':
					echo '<input class="cmb_text_small" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'text_medium':
					echo '<input class="cmb_text_medium" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'text_date':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'text_date_timestamp':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr(date( 'm\/d\/Y', $meta )) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				...
				case 'text_datetime_timestamp':
					...
					echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '[time]" id="', esc_attr($field['id']), '_time" value="', '' !== $meta ? esc_attr(date( 'h:i A', $meta )) : esc_attr($field['std']), '" /><span class="cmb_metabox_description" >', wp_kses_post($field['desc']), '</span>';
					break;
				case 'text_time':
					echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'text_money':
					echo '$ <input class="cmb_text_money" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'colorpicker':
					...
					elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
						$meta = "#";
					echo '<input class="cmb_colorpicker cmb_text_small" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', esc_attr($meta), '" /><span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'textarea':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="10">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				case 'textarea_small':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="4">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				case 'textarea_code':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="8" class="cmb_textarea_code">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				case 'select':
					...
					foreach ($field['options'] as $option) {
						...
						echo '<option value="'.esc_attr($option['value']).'" '.selected($default, $option['value']).'>'.esc_html($option['name']).'</option>';
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				case 'select_slider_cats':
				
                    $ts_slider_categories = ts_essentials_get_categories('slider-category', 0, 'cb_metabox_array');
                    ...
                    */
					...
					foreach ($ts_slider_categories as $option) {
                        $selected = ($default == $option['value']) ? 'selected="selected"' : '';
						echo '<option value="'.esc_attr($option['value']).'" '.selected($default, $option['value']).'>'.$option['name'].'</option>';
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
					break;
				...
				case 'checkbox':
					...
					echo '<span class="cmb_metabox_description">', wp_kses_post($field['desc']), '</span>';
					break;
				case 'multicheck':
					echo '<ul>';
					$i = 1;
					...
					break;
				case 'title':
					echo (trim($field['name'])) ? '<h5 class="cmb_metabox_title">'. esc_html($field['name']). '</h5>' : '';
					echo (trim($field['desc'])) ? '<p class="cmb_metabox_description">'. wp_kses_post($field['desc']). '</p>' : '';
					break;
				case 'wysiwyg':
				...
				case 'taxonomy_radio':
					...
					foreach ( $terms as $term ) {
						if ( !is_wp_error( $names ) && !empty( $names ) && !strcmp( $term->slug, $names[0]->slug ) ) {
							...
							echo '<li><input type="radio" name="', esc_attr($field['id']), '" value="' . esc_attr($term->slug) . '  ' , $meta == $term->slug ? $meta : ' ' ,'  ">' . esc_html($term->name) .'</li>';
						}
					}
					echo '</ul>';
					echo '<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
				...
				case 'taxonomy_multicheck':
					...
					foreach ($terms as $term) {
                        ...
                        if (in_array( $term->term_id, $meta )){ echo ' checked="checked" ';}
				...
				break;
				case 'oembed':
					echo '<input class="cmb_oembed" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" />','<p class="cmb_metabox_description">', wp_kses_post($field['desc']), '</p>';
