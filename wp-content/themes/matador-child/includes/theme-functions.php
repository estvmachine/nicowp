...
function ts_smart_date($date, $format = '', $post = null) {
    if(ts_option_vs_default('smart_datetime', 0) == 1) {
        ...
        if($date_pref != 'american')
        {
            ...
            elseif($format == 'F j')
                $format = 'j F';
            elseif($format == 'M j Y' || $format == 'M j, Y')
                $format = 'j M Y';
            elseif($format == 'F j Y' || $format == 'F j, Y')
                $format = 'j F Y';
...
function ts_post_direction_nav($reverse = false, $previous_text = '', $next_text = '')
{
    ...
    $prev_function('%link', '<span class="dir smaller uppercase">'.$previous_text.'<i class="fa fa-chevron-right"></i></span><span class="title">%title</span>');
    echo '</div>';
    echo '</div>';
}
